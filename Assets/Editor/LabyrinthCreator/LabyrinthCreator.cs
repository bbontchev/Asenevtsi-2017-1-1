...
class MazeBuilder : EditorWindow {
	...
	private void createSlide(AssemblyCSharpEditor.Slide slide, string slideName, string roomName, GameObject slidePlane)
	{
		...
		if (slide.Image != null ) {
			if (slide.Image != "NONE") { //BB When slide.Image == "NONE" then no background will be benerated
	...
	private void createGame(AssemblyCSharpEditor.Room room)
	{
		...
		if (room.Game != null && room.Game.gameElements != null && room.Game.gameElements.Count > 0) {
			foreach (AssemblyCSharpEditor.GameElement gameElement in room.Game.gameElements) {
				int x;
				...
				if (gameElement.Type == "Ball") {
					...
					if (room.Game.MinPoints > 0) {
						//newElement.transform.FindChild ("Ball").gameObject.SendMessage ("setMinPoints", room.Game.MinPoints);
					}
				}
			...
			foreach (AssemblyCSharpEditor.GameElement gameElement in room.Game.gameElements) {
				...
				if (gameElement.Type == "Circle") {
					...
					if (gameElement.Ball != null && gameElement.Ball != "") {
						GameObject ball = GameObject.Find("/" + room.Name + "/Game/" + gameElement.Ball + "/Ball");
